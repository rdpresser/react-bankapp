...
{
    public class TakeLoanOperationHandler : IRequestHandler<TakeLoanOperationCommand, Result<LoanDataResponse>>
    {
        private readonly IUnitOfWork _unitOfWork;
        private readonly IAccountService _accountService;
        private readonly ITransactionService _transactionService;
        private readonly ILoanService _loanService;
        private readonly IBaseValidation<TakeLoanOperationCommand> _validator;

        public TakeLoanOperationHandler(IUnitOfWork unitOfWork, IAccountService accountService, ITransactionService transactionService,
            ILoanService loanService, IBaseValidation<TakeLoanOperationCommand> validator)
        {
        ...
        public async Task<Result<LoanDataResponse>> Handle(TakeLoanOperationCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                if (account == null)
                {
                    return Result<LoanDataResponse>.Failure(new Dictionary<string, string[]> { { "TakeLoanOperationCommand", ["Account not found"] } });
                }
                ...
                if (!commitResult)
                {
                    return Result<LoanDataResponse>.Failure(new Dictionary<string, string[]> { { "TakeLoanOperationCommand|Commit", ["Failed to commit transaction"] } });
                }
            ...
            {
                return Result<LoanDataResponse>.Failure(new Dictionary<string, string[]> { { "TakeLoanOperationCommand", [ex.Message] } });
            }
        }
    }
}
